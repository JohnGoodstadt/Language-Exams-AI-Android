package com.goodstadt.john.language.exams.screens.Conjugations
           ...
           // val result99= vocabRepository.loadBundledFormat0Data(bundle_file_name) //direct from bundle
            ...
            result.onSuccess { vocabFile ->
               ...
               // val cachedKeys = vocabRepository.getSentenceKeysWithCachedAudio(vocabFile.categories, currentVoiceName)
                ...
                _uiState.value = ConjugationsUiState.Success(
                    vocabFile.categories,
                    selectedVoiceName = currentVoiceName,
                    currentSheetName = firestore_sheet_name
                )
            }.onFailure { error ->
                _uiState.value =
                    ConjugationsUiState.Error(
                        error.localizedMessage ?: "Failed to load file $bundle_file_name"
                    )
            }
        }
    }
