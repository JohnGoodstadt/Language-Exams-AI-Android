...
data class CategoryTabOriginalUiState(
...
sealed interface UiOriginalEvent {
    data class ShowSnackbar(val message: String, val actionLabel: String? = null) : UiOriginalEvent
    // You can add other one-off events here later
}
@HiltViewModel
class CategoryTabViewModelOriginal @Inject constructor(
...
//    private val ttsCreditsRepository: TtsCreditsRepository,
    ...
    private val _uiState = MutableStateFlow(CategoryTabOriginalUiState())
...
//                    ttsStatsRepository.printStats(TTSStatsRepository.fsDOC.WORDSTATS)
    ...
    fun resetState() {
        ...
        _uiState.value = CategoryTabOriginalUiState()
    }
    fun saveDataOnExit() {
...
//        Timber.d("progressStats: ${ttsStatsRepository.progressStats}")
    ...
    /**
     * Calculates stats for the Gamification Sheet (Tabs 1-3).
     * Returns (Heard, Total)
     */
    fun calculateGrandTotals(): Pair<Int, Int> {
        // We delegate to AudioCacheManager which has the global view
    ...
    fun buildCategoryProgress(): List<CategoryProgress> {
        // Return sorted list for the sheet
